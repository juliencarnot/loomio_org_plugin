      ...
      setup! :loomio_org_plugin do |plugin|
        ...
        plugin.extend_class API::MembershipsController do
          ...
          def include_emails?
            ...
            @group.parent_or_self.subscription&.is_paid?
          end
        end
        ...
        plugin.use_events do |event_bus|
          event_bus.listen('group_create')  do |group|
            SubscriptionService.new(group).start_gift! if group.is_parent?
          end

          event_bus.listen('group_archive') do |group|
            SubscriptionService.new(group).end_subscription! if group.is_parent?
          end
        end
